...
# 创建汇总表模板
demo = ts.get_hist_data('600000', start=start_time)
demo = demo.sort_index(ascending=True)
demoT = demo.T
dateName = demoT.columns.tolist()
...
for sheet in sheets:
    ...
    for SC in stockCode:
        ...
        # 将数据加入汇总表
        newRow = df['DB2']
        newRow = newRow[start_time:]
        newRow.name = code
        conclude = conclude.append(newRow)
        print("代码", code, "已记录")

print(conclude)
saveFile = savePath + "/conclusion.csv"
